...
VSeqNext(VectorScanState *vss)
{
	...
	if (scandesc == NULL)
	{
		 ...
		 */
		scandesc = (HeapScanDesc) heap_beginscan(node->ss.ss_currentRelation,
								  estate->es_snapshot,
								  0, NULL, NULL, SO_TYPE_SEQSCAN);
		node->ss.ss_currentScanDesc = (TableScanDesc) scandesc;
	}
	...
	for (row = 0 ; row < BATCHSIZE; row++)
	{
		 ...
		 */
		tuple = heap_getnext((TableScanDesc) scandesc, direction);
...
VExecEndSeqScan(VectorScanState *vss)
{
	 ...
	 */
	if (scanDesc != NULL)
		heap_endscan((TableScanDesc) scanDesc);
}
